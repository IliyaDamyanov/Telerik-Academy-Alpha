...
namespace Sorting
{
    static class QuickSortClass
    {
            ...
            where T : IComparable<T>
        {
            ...
            int pivotIndex = 0;
            if(array[left].CompareTo(array[right - 1]) < 0 ^ array[left].CompareTo(array[middle]) < 0)
            {
                pivotIndex = left;
            }
            else if(array[right - 1].CompareTo(array[left]) < 0 ^ array[right - 1].CompareTo(array[middle]) < 0)
            {
                pivotIndex = right;
            }
            else
